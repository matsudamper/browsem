...
- PR の監視のために定期実行のセルフチェックインをスケジュールしない。CI 完了やレビューコメントは PR のイベント通知で届くため、通知が来たときにだけ対応する
  - Web 環境（リモート実行環境）では `.claude/hooks/deny-scheduled-checkin.sh` が `CronCreate` / `ScheduleWakeup` / `create_trigger` / `fire_trigger` を PreToolUse で拒否する
  - ローカル CLI や IDE 拡張では制限しない

### PR の自動更新（Claude Web 環境のみ）

このルールは Claude Code の Web 環境（リモート実行環境）では自動的に適用する。ローカル CLI や IDE 拡張ではユーザーから指示があった場合にのみ適用する。
