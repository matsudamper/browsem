...
このルールは Claude Code の Web 環境（リモート実行環境）でのみ適用する。ローカル CLI や IDE 拡張では適用しない。
また、自動では実行せず、ユーザーから PR の更新指示があった場合にのみこのルールに従う。

ユーザーから指示があった場合、現在のブランチに紐づく既存の PR を確認し、以下の手順でタイトルと本文の更新要否を判断する。
