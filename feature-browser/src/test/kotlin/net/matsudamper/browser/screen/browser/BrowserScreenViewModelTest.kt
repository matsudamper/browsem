...
class BrowserScreenViewModelTest {
    ...
    fun `同じグループ内のタブのみがswipePreviewに反映される`() = runTest(testDispatcher) {
        ...
        assertEquals("a", uiState.swipePreview.previousTab?.content?.title)
        assertEquals("c", uiState.swipePreview.nextTab?.content?.title)
    }
    ...
    fun `browserTabsFlowの変更がuiStateに反映される`() = runTest(testDispatcher) {
        ...
        // 初期状態: a の次は b
        assertEquals("b", viewModel.uiState.value.swipePreview.nextTab?.content?.title)
